...
// Fetch marks data
const getMarksData = async (courseId, deptId, academicYear,sem) => {
  // Construct dynamic student table name
  ...
  const query = `
    SELECT 
      ...
      m.sem
    FROM marks m
    JOIN ${studentTableName} s 
      ON m.roll_no = s.roll_no
    WHERE m.course_id = ? AND m.dept_id = ? AND m.academic_yr = ? AND m.sem = ?
  `;

  const [results] = await db.query(query, [courseId, deptId, academicYear, sem]);
  return results;
};
